...
font_size = 10
ax.text(0.05, 0.95, 'Total Hydro 2019', fontsize=font_size, color=color_total_old, horizontalalignment='left', verticalalignment='center', transform=ax.transAxes)

ax.tick_params(axis='both', which='major', direction='in' )
ax.tick_params(axis='both', which='minor', direction='in' )

ax.set_xlim(0.7, 1E5)
ax.set_ylim(7E-3, 1E3)

ax.set_yscale('log')
ax.set_xscale('log')

fig.suptitle('MHD Weak Scaling on Frontier', fontsize=font_size, alpha=alpha)
ax.set_ylabel(r'Milliseconds / 256$^3$ Cells / GPU', fontsize=font_size, alpha=alpha)
